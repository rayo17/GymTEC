...
import EliminarPuestoFormulario from '../Forms/EliminarPuestoFormulario';
...
render() {
  const { puestos, error, showDialog, showtwoDialog, showthreeDialog } = this.state;
  ...
  <h1 style={{ margin: '50px 0', fontSize: '2.5rem', fontWeight: 'bold', textTransform: 'uppercase' }}>Puestos de empleados</h1>
      ...
      onClick={this.toggletD}>Editar puesto</button>
      {showtwoDialog && (
          <div
            style={{
              position: "fixed",
      ...
      onClick={this.togglethD}>Eliminar puesto</button>
      {showthreeDialog && (
          <div
            style={{
              position: "fixed",
          ...
          >
            {/* contenido del diálogo */}
            <EliminarPuestoFormulario 
              onClose={this.togglethD}
              onDeletePuesto={this.handlePuesto}
            />
